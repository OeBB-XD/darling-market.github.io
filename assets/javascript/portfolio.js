...
$(document).ready(() => {
  ...
  app.titleChanger(['Darling Market']);
});
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Bienvenue...', "Votre identité : <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    const country = data.country_name ? data.country_name : 'votre pays';

    writeLine([`Acces autorise! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Content de te revoir ! <span style='font-size: 14px; color: #06d;'>[Connected !]</span>! `], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>DarlingMarket.xyz <3</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
